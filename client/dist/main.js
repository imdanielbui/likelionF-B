...
const createDrinkList = async (index = 0) => {
...
function handleAddOrderBtn(drink) {
    const order = {
    ...
    const isHasDrink = orderList.find((d) => d.drinkId === order.drinkId && d.size === order.size);
    if (isHasDrink) {
        isHasDrink.quantity++;
        return;
    }
...
async function createItemOrder(order) {
...
function handleRenderListOrder() {
    ...
    if (totalPriceValue)
        totalPriceValue.textContent = `${total} VND`;
}
function handleQuantity(isPlus, quantityValue, currentQuantity, order, event) {
...
function deleteAll() {
    ...
    if (containerMenu)
        containerMenu.innerHTML = "";
    if (totalPriceValue)
        totalPriceValue.textContent = "0 VND";
}
